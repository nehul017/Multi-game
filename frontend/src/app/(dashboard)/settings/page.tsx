...
export default function SettingsPage() {
  ...
  const handleSaveProfile = async () => {
    try {
      await updateProfile.mutateAsync({ username, email, bio });
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
              ...
              {Object.entries(notifications).map(([key, value]) => (
                <div key={key} className="flex items-center justify-between py-2 border-b border-surface-lighter/20 last:border-0">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Receive notifications for {key.replace(/([A-Z])/g, ' $1').toLowerCase()}</p>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between py-2 border-b border-surface-lighter/20">
                <div>
                  ...
                  <p className="text-xs text-gray-400">Allow anyone to view your profile</p>
                </div>
                <Switch checked={true} onChange={() => {}} />
              </div>
              <div className="flex items-center justify-between py-2 border-b border-surface-lighter/20">
                <div>
                  ...
                  <p className="text-xs text-gray-400">Receive friend requests from anyone</p>
                </div>
                <Switch checked={true} onChange={() => {}} />
              </div>
              <div className="flex items-center justify-between py-2">
                <div>
                  <p className="text-sm font-medium text-white">Show Match History</p>
                  <p className="text-xs text-gray-400">Display match history on your profile</p>
                </div>
                <Switch checked={true} onChange={() => {}} />
              </div>
              <Button leftIcon={<Save className="w-4 h-4" />} onClick={handleSavePreferences} className="mt-4">Save Changes</Button>
        ...
        {activeTab === 'sound' && (
          <Card>
            <h3 className="text-lg font-semibold text-white mb-6">Sound Settings</h3>
            <div className="space-y-4">
